...
func NewRabbitMQ() *RabbitMQ {
	...
	/*"dead-letter exchange" (DLX) no RabbitMQ. Uma DLX é uma característica
	do RabbitMQ que permite que mensagens que não podem ser entregues a
	uma fila sejam encaminhadas para outra fila ou exchange para
	processamento posterior ou para serem descartadas.
	*/
...
func (r *RabbitMQ) Consume(messageChannel chan amqp.Delivery) {
	...
	/*
		Este trecho de código em Go é um exemplo de uma goroutine
		anônima que é executada em paralelo com o restante do programa.
	*/
	go func() {
